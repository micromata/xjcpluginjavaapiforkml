...
/**
 * Modifies the JAXB code model to set default values to the schema "default" attribute. Currently, the following field types can be
 * initialized:
 * <ul>
 ...
 * @version 1.1
 */
public class DefaultValuePlugin extends Command {
	private static final Logger LOG = Logger.getLogger(DefaultValuePlugin.class.getName());

	public DefaultValuePlugin(Outline outline, Options opts, ErrorHandler errorHandler, ClazzPool pool) {
	 ...
	 * <li>The XSD description is of type xsd:element (code level default values are not necessary for fields generated from attributes)</li>
	...
	public boolean run(Outline outline, Options opt, ErrorHandler errorHandler) {
		...
		for (ClassOutline co : outline.getClasses()) {
			...
			for (FieldOutline f : co.getDeclaredFields()) {
				...
				if ("java.lang.String".equals(typeFullName)) {
				...
				else if (("java.lang.Byte".equals(typeFullName)) || ("java.lang.Short".equals(typeFullName)) || ("java.lang.Integer"
				    .equals(typeFullName))) {
				...
				else if (("java.lang.Single".equals(typeFullName)) || ("java.lang.Double".equals(typeFullName))) {
					...
					var.init(JExpr.invoke(dtf, "newXMLGregorianCalendar").arg(defaultValue));
					if (opt.verbose)
						System.out
						    .println("[INFO] Initializing XMLGregorianCalendar variable " + fieldInfo.displayName() + " with value of " + defaultValue);
				}
				...
				// Don't know how to create default for this type
					...
					System.out
					    .println("[WARN] Did not create default value for field " + fieldInfo.displayName() + ". Don't know how to create default value expression for fields of type " + typeFullName + ". Default value of \"" + defaultValue + "\" specified in schema");
				}
	...
	/**
	 * Enhance the CodeModel of a Class to include a {@link DatatypeFactory} as a static private field. The factory is needed to construct
