...
public class CreateMarshalAndUnmarshal extends Command {
	...
	public CreateMarshalAndUnmarshal(final Outline outline, final Options opts, final ErrorHandler errorHandler,
			final ClazzPool pool) {
	...
	private void generateMarshallerAndUnMarshaller(final ClassOutlineImpl cc) {
		...
		namespaceFilterHandler = classFactory.createClass(kmlpackage, JMod.FINAL, "NamespaceFilterHandler", null,
				ClassType.CLASS);
		namespaceFilterHandler._implements(contentHandlerClass.boxify());
		final JFieldVar KML_20 = namespaceFilterHandler.field(JMod.PRIVATE | JMod.STATIC | JMod.FINAL, stringClass,
				"KML_20", JExpr.lit("http://earth.google.com/kml/2.0"));
		final JFieldVar KML_21 = namespaceFilterHandler.field(JMod.PRIVATE | JMod.STATIC | JMod.FINAL, stringClass,
				"KML_21", JExpr.lit("http://earth.google.com/kml/2.1"));
		final JFieldVar KML_22 = namespaceFilterHandler.field(JMod.PRIVATE | JMod.STATIC | JMod.FINAL, stringClass,
				"KML_22", JExpr.lit("http://www.opengis.net/kml/2.2"));
		...
		final JMethod startElement = namespaceFilterHandler.method(JMod.PUBLIC, cc.implClass.owner().VOID,
				"startElement");
		...
		final JConditional if1 = startElement.body()
				._if(uri.invoke("equals").arg(KML_20).cor(uri.invoke("equals").arg(KML_21)));
		...
		final JMethod endDocument = namespaceFilterHandler.method(JMod.PUBLIC, cc.implClass.owner().VOID,
				"endDocument");
		...
		endElement.body()
				.add(content.invoke("endElement").arg(endElementUri).arg(endElementLocalName).arg(endElementqName));

		final JMethod endPrefixMapping = namespaceFilterHandler.method(JMod.PUBLIC, cc.implClass.owner().VOID,
				"endPrefixMapping");
		...
		final JMethod ignorableWhitespace = namespaceFilterHandler.method(JMod.PUBLIC, cc.implClass.owner().VOID,
				"ignorableWhitespace");
		...
		ignorableWhitespace.body().add(content.invoke("ignorableWhitespace").arg(ignorableWhitespaceCh)
				.arg(ignorableWhitespaceStart).arg(ignorableWhitespaceLength));

		final JMethod processingInstruction = namespaceFilterHandler.method(JMod.PUBLIC, cc.implClass.owner().VOID,
		...
		processingInstruction.body().add(content.invoke("processingInstruction").arg(processingInstructionTarget)
				.arg(processingInstructionData));

		final JMethod setDocumentLocator = namespaceFilterHandler.method(JMod.PUBLIC, cc.implClass.owner().VOID,
				"setDocumentLocator");
		...
		final JMethod skippedEntity = namespaceFilterHandler.method(JMod.PUBLIC, cc.implClass.owner().VOID,
				"skippedEntity");
		...
		final JMethod startDocument = namespaceFilterHandler.method(JMod.PUBLIC, cc.implClass.owner().VOID,
				"startDocument");
		startDocument._throws(saxExceptionClass.boxify());
		startDocument.body().add(content.invoke("startDocument"));

		final JMethod startPrefixMapping = namespaceFilterHandler.method(JMod.PUBLIC, cc.implClass.owner().VOID,
				"startPrefixMapping");
		...
		startPrefixMapping.body()
				.add(content.invoke("startPrefixMapping").arg(startPrefixMappingPrefix).arg(startPrefixMappingUri));

		namespaceFilterXMLReaderclass = classFactory.createClass(kmlpackage, JMod.FINAL, "NamespaceFilterXMLReader",
				null, ClassType.CLASS);
		...
		final JVar paramValidate = namespaceFilterXMLReaderclassConstrutor.param(cc.implClass.owner().BOOLEAN,
				"validate");
		namespaceFilterXMLReaderclassConstrutor._throws(saxExceptionClass.boxify())
				._throws(parserConfigurationExceptionClass.boxify());
		final JVar parserFactory = namespaceFilterXMLReaderclassConstrutor.body().decl(saxParserFactoryClass,
				"parserFactory", saxParserFactoryClass.boxify().staticInvoke("newInstance"));
		...
		namespaceFilterXMLReaderclassConstrutor.body().assign(xmlReader,
				parserFactory.invoke("newSAXParser").invoke("getXMLReader"));

		final JMethod getContentHandler = namespaceFilterXMLReaderclass.method(JMod.PUBLIC, ContentHandler.class,
				"getContentHandler");
		getContentHandler.body()._return(xmlReader.invoke("getContentHandler"));

		final JMethod getDTDHandler = namespaceFilterXMLReaderclass.method(JMod.PUBLIC, DTDHandler.class,
				"getDTDHandler");
		getDTDHandler.body()._return(xmlReader.invoke("getDTDHandler"));

		final JMethod getEntityResolver = namespaceFilterXMLReaderclass.method(JMod.PUBLIC, EntityResolver.class,
				"getEntityResolver");
		getEntityResolver.body()._return(xmlReader.invoke("getEntityResolver"));

		final JMethod getErrorHandler = namespaceFilterXMLReaderclass.method(JMod.PUBLIC, ErrorHandler.class,
				"getErrorHandler");
		getErrorHandler.body()._return(xmlReader.invoke("getErrorHandler"));

		final JMethod getFeature = namespaceFilterXMLReaderclass.method(JMod.PUBLIC, cc.implClass.owner().BOOLEAN,
				"getFeature");
		...
		final JMethod setContentHandler = namespaceFilterXMLReaderclass.method(JMod.PUBLIC, cc.implClass.owner().VOID,
				"setContentHandler");
		final JVar setContentHandlerHandler = setContentHandler.param(ContentHandler.class, "handler");
		setContentHandler.body().add(xmlReader.invoke("setContentHandler")
				.arg(JExpr._new(namespaceFilterHandler).arg(setContentHandlerHandler)));

		final JMethod setDTDHandler = namespaceFilterXMLReaderclass.method(JMod.PUBLIC, cc.implClass.owner().VOID,
				"setDTDHandler");
		...
		final JMethod setEntityResolver = namespaceFilterXMLReaderclass.method(JMod.PUBLIC, cc.implClass.owner().VOID,
				"setEntityResolver");
		...
		final JMethod setErrorHandler = namespaceFilterXMLReaderclass.method(JMod.PUBLIC, cc.implClass.owner().VOID,
				"setErrorHandler");
		...
		final JMethod setFeature = namespaceFilterXMLReaderclass.method(JMod.PUBLIC, cc.implClass.owner().VOID,
				"setFeature");
		...
		final JMethod setProperty = namespaceFilterXMLReaderclass.method(JMod.PUBLIC, cc.implClass.owner().VOID,
				"setProperty");
	...
	private void generateHelperMethods(final ClassOutlineImpl cc) {
		...
		varMissingNameCounter = cc.implClass.field(JMod.PRIVATE | JMod.TRANSIENT, cc.implClass.owner().INT,
				"missingNameCounter", JExpr.direct("1"));
		// private static Unmarshaller u = null;
		// uVar = cc.implClass.field(JMod.PRIVATE | JMod.TRANSIENT,
		// jaxbUnmarshallerClass, "u", JExpr._null());
		...
		ifBlockOutstream._then().assign(jcVar,
				jaxbContextClass.boxify().staticInvoke("newInstance").arg(JExpr.direct("Kml.class")));
		...
		ifBlockMarshaller._then().add(mVar.invoke("setProperty")
				.arg(jaxbMarshallerClass.boxify().staticRef("JAXB_FORMATTED_OUTPUT")).arg(JExpr.TRUE));
		...
		try {
			...
			ifBlockMarshaller._then().add(mVar.invoke("setProperty").arg("com.sun.xml.bind.namespacePrefixMapper")
					.arg(JExpr._new(createNestedPrefixCustomizer)));
		...
		// return u;
		// }
		// createUnmarshaller = cc.implClass.method(JMod.PRIVATE, jaxbUnmarshallerClass,
		// "createUnmarshaller");
		// createUnmarshaller._throws(jaxbExceptionClass.boxify());
		// final JConditional ifBlockUnmarshaller =
		// createUnmarshaller.body()._if(uVar.eq(JExpr._null()));
		// ifBlockUnmarshaller._then().assign(uVar,
		// JExpr._this().invoke(getJAXBContex).invoke("createUnmarshaller"));
	...
	private void createAddToKmzFile(final ClassOutlineImpl cc) {
		...
		final JConditional if1 = createAddToKmzFile.body()
				._if(varKmzFile.invoke("getFeature").eq(JExpr._null())
						.cor(varKmzFile.invoke("getFeature").invoke("getName").eq(JExpr._null()))
						.cor(varKmzFile.invoke("getFeature").invoke("getName").invoke("length").eq(JExpr.lit(0))));
		if1._then().assign(varFileName,
				JExpr.lit("noFeatureNameSet").plus(varMissingNameCounter.incr()).plus(JExpr.lit(".kml")));
		if1._else().assign(varFileName, varKmzFile.invoke("getFeature").invoke("getName"));
		if1._else()._if(varFileName.invoke("endsWith").arg(".kml").not())._then().assignPlus(varFileName,
				JExpr.lit(".kml"));

		createAddToKmzFile.body()._if(varMainFile)._then().assign(varFileName, JExpr.lit("doc.kml"));
		createAddToKmzFile.body().add(varOut.invoke("putNextEntry").arg(JExpr._new(zipEntryClass)
				.arg(UrlEncoderClass.boxify().staticInvoke("encode").arg(varFileName).arg(JExpr.lit("UTF-8")))));
	...
	private JDefinedClass createNestedPrefixCustomizer(final ClassOutlineImpl cc) throws JClassAlreadyExistsException {
		final JDefinedClass namespacebeautyfier = cc.implClass._class(JMod.PRIVATE | JMod.STATIC | JMod.FINAL,
				"NameSpaceBeautyfier");
		namespacebeautyfier._extends(NamespacePrefixMapper.class);

		final JMethod namespacebeautyfiergetpreferredprefix = namespacebeautyfier.method(JMod.PUBLIC, stringClass,
				"getPreferredPrefix");
		...
		namespacebeautyfiergetpreferredprefix.javadoc()
				.append("<p>Customizing Namespace Prefixes During Marshalling to a more readable format.</p>\n");
		...
		namespacebeautyfiergetpreferredprefix.javadoc()
				.append("<p>namespaceUri: http://www.w3.org/2005/Atom              prefix: atom</p>");
		namespacebeautyfiergetpreferredprefix.javadoc()
				.append("<p>namespaceUri: urn:oasis:names:tc:ciq:xsdschema:xAL:2.0 prefix: xal</p>");
		namespacebeautyfiergetpreferredprefix.javadoc()
				.append("<p>namespaceUri: http://www.google.com/kml/ext/2.2        prefix: gx</p>");
		...
		namespacebeautyfiergetpreferredprefix.body()
				._if(namespaceuri.invoke("matches").arg("http://www.w3.org/\\d{4}/Atom"))._then()
				._return(JExpr.lit("atom"));
		namespacebeautyfiergetpreferredprefix.body()
				._if(namespaceuri.invoke("matches").arg("urn:oasis:names:tc:ciq:xsdschema:xAL:.*?"))._then()
				._return(JExpr.lit("xal"));
		namespacebeautyfiergetpreferredprefix.body()
				._if(namespaceuri.invoke("matches").arg("http://www.google.com/kml/ext/.*?"))._then()
	 ...
	 * Java to KML
	 * 
	 * public boolean marshal(final String filename, boolean zipped) throws
	 * FileNotFoundException { OutputStream out = new FileOutputStream(filename); if
	 * (zipped) { out = new ZipOutputStream(out); }
	 * 
	 * return this.marshall(out); }
	 * 
	 * public boolean marshal(OutputStream outputStream) { try { jc =
	 * this.createMarshaller(); JAXBElement&lt;Kml&gt; jaxbRootElement = new
	 * JAXBElement&lt;Kml&gt;(new QName(&quot;http://www.opengis.net/kml/2.2&quot;,
	 * &quot;kml&quot;), Kml.class, this);
	 * m.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
	 * m.marshal(jaxbRootElement, outputStream); return true; } catch (JAXBException
	 * _x) { _x.printStackTrace(); return false; } }
	 * 
	 * 
	 * @param cc
	 */
	private void generateMarshalMethods(final ClassOutlineImpl cc) {
		...
		// JAXBElement<Kml> jaxbRootElement = new JAXBElement<Kml>(new
		// QName("http://www.opengis.net/kml/2.2", "kml"), Kml.class, this);
		// m.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
		...
		// public boolean marshal(final File filename, boolean zipped) throws
		...
		comment.add("The object is not saved as a zipped .kmz file.\n");
		comment.add("@see marshalKmz(String, Kml...)");
		final JMethod generateMarshallFilenameWithZIP = cc.implClass.method(JMod.PUBLIC, cc.implClass.owner().BOOLEAN,
				"marshal");
		...
		// final JVar zippedVar = generateMarshallFilenameWithZIP.param(JMod.FINAL,
		// boolean.class, "zipped");
		final JVar outVar = generateMarshallFilenameWithZIP.body().decl(outputStreamClass, "out",
				JExpr._new(fileOutputStreamClass).arg(filenameVar));
		// final JConditional ifBlockFilename =
		// generateMarshallFilenameWithZIP.body()._if(zippedVar.eq(JExpr.TRUE));
		// ifBlockFilename._then().assign(outVar,
		// JExpr._new(zipOutputStreamClass).arg(outVar));
		...
		// comment.add("The object is not saved as a zipped .kmz file.\n");
		// comment.add("@see marshalKmz(String, Kml...)");
		// final JMethod generateMarshallFilename = cc.implClass.method(JMod.PUBLIC,
		// cc.implClass.owner().BOOLEAN, "marshal");
		// generateMarshallFilename._throws(FileNotFoundException.class);
		// generateMarshallFilename.javadoc().append(comment);
		// JVar filename = generateMarshallFilename.param(JMod.FINAL, File.class,
		// "filename");
		//
	...
	private JMethod generateMarshal(final ClassOutlineImpl cc, final ArrayList<String> comment) {
		...
		final JMethod generateMarshalOutputStream = cc.implClass.method(JMod.PUBLIC, cc.implClass.owner().BOOLEAN,
				"marshal");
		...
		// JAXBElement<Kml> jaxbRootElement = new JAXBElement<Kml>(new
		// QName("http://www.opengis.net/kml/2.2", "kml"), Kml.class, this);
		// JInvocation newJaxbElement =
		// JExpr._new(jaxbElementClass).arg(createQName("http://www.opengis.net/kml/2.2",
		// "kml")).arg(
		// kmlClass.boxify().dotclass()).arg(JExpr._this());
		// final JVar jaxbRootElementVar = tryBlock.body().decl(jaxbElementClass,
		// "jaxbRootElement", newJaxbElement);
	...
	private JMethod generateMarshal(final ClassOutlineImpl cc, final JType argumentType,
			final ArrayList<String> comment) {
		// public boolean marshall(final String filename) throws FileNotFoundException {
		final JMethod generateMarshalOutputStream = cc.implClass.method(JMod.PUBLIC, cc.implClass.owner().BOOLEAN,
				"marshal");
		...
		generateMarshalOutputStream.javadoc().append(comment);
		final JVar value = generateMarshalOutputStream.param(JMod.FINAL, argumentType,
				argumentType.name().toLowerCase());
		...
		// } finally {
		if (hasCloseableInterface(argumentType)) {
			final JTryBlock tryClose = tryBlock._finally()._try();
			tryClose.body().add(value.invoke("close"));

			final JBlock closeCatch = tryClose._catch(codeModel.ref(IOException.class)).body();
			closeCatch.directStatement("_x.printStackTrace();");
			closeCatch._return(JExpr.FALSE);
		}
		// }
		return generateMarshalOutputStream;
	}

	private boolean hasCloseableInterface(final JType argumentType) {
		try {
			final Class<?>[] interfaces = getClass().getClassLoader().loadClass(argumentType.binaryName())
					.getInterfaces();
			Optional<Class<?>> findFirst = Arrays.stream(interfaces).filter(inf -> Closeable.class.equals(inf))
					.findFirst();
	...
	private void generateUnMarshalMethods(final ClassOutlineImpl cc) {
		schemaLocationVar = cc.implClass.field(JMod.PRIVATE | JMod.STATIC | JMod.FINAL, stringClass, "SCHEMA_LOCATION",
				JExpr.lit("src/main/resources/schema/ogckml/ogckml22.xsd"));
	...
	/**
	 * public static Kml unmarshalLegacyKml(File source) throws
	 * FileNotFoundException { try { Unmarshaller unmarshaller =
	 * JAXBContext.newInstance((Kml.class)).createUnmarshaller(); XMLReader reader =
	 * new NamespaceFilterXMLReader(); InputSource is = new InputSource(new
	 * FileReader(source)); SAXSource ss = new SAXSource(reader, is); return (Kml)
	 * unmarshaller.unmarshal(ss); } catch (SAXException _x) { _x.printStackTrace();
	 * } catch (ParserConfigurationException _x) { _x.printStackTrace(); } catch
	 * (JAXBException _x) { _x.printStackTrace(); } return null; }
	 */
	private void generateUnmarshalLegacyKmlMethod(final ClassOutlineImpl cc) {
		final JMethod generateUnMarshallerFromString = cc.implClass.method(JMod.PUBLIC | JMod.STATIC, kmlClass,
				"unmarshalLegacyKml");
		generateUnMarshallerFromString.javadoc().add("KML to Java\n");
		generateUnMarshallerFromString.javadoc().add("reads legacy kml files. \n");
		generateUnMarshallerFromString.javadoc()
				.add("Supported are KML 2.0 (namespace: http://earth.google.com/kml/2.0) \n");
		generateUnMarshallerFromString.javadoc()
				.add("          and KML 2.1 (namespace: http://earth.google.com/kml/2.1) \n");
		...
		final JVar localUnmarshallerFile = tryBlock.body().decl(jaxbUnmarshallerClass, "unmarshaller", jaxbContextClass
				.boxify().staticInvoke("newInstance").arg(JExpr.direct("Kml.class")).invoke("createUnmarshaller"));

		final JVar inputsource = tryBlock.body().decl(inputsourceClass, "input",
				JExpr._new(inputsourceClass).arg(JExpr._new(fileReaderClass).arg(fileunmarshallVar)));
		final JVar saxsource = tryBlock.body().decl(saxsourceClass, "saxSource", JExpr._new(saxsourceClass)
				.arg(JExpr._new(namespaceFilterXMLReaderclass).arg(JExpr.FALSE)).arg(inputsource));
		final JVar decl = tryBlock.body().decl(kmlClass, "jaxbRootElement",
	...
	private void generateUnmarshalFromKmzMethod(final ClassOutlineImpl cc) {
		final JMethod unmarshalFromKMZ = cc.implClass.method(JMod.PUBLIC | JMod.STATIC, kmlClass.array(),
				"unmarshalFromKmz");
		unmarshalFromKMZ.javadoc().add("KMZ to Java\n");
		unmarshalFromKMZ.javadoc().add("Similar to the other unmarshal methods\n\n");
		unmarshalFromKMZ.javadoc()
				.add("with the exception that it transforms a KMZ-file into a graph of Java objects. \n");
		...
		final JVar varEMPTY_KML_ARRAY = unmarshalFromKMZ.body().decl(kmlClass.array(), "EMPTY_KML_ARRAY",
				JExpr.direct("new Kml[0]"));// JExpr._new(kmlClass.array().)..arg("0"));
		unmarshalFromKMZ.body()._if(varFile.invoke("getName").invoke("endsWith").arg(".kmz").not())._then()
				._return(varEMPTY_KML_ARRAY);
		final JVar varZip = unmarshalFromKMZ.body().decl(zipFileClass, "zip", JExpr._new(zipFileClass).arg(varFile));
		final JVar varEntries = unmarshalFromKMZ.body().decl(
				enumerationClass.boxify().narrow(zipEntryClass.boxify().wildcard()), "entries",
				varZip.invoke("entries"));
		unmarshalFromKMZ.body()._if(varFile.invoke("exists").not())._then()._return(varEMPTY_KML_ARRAY);
		final JVar varKmlFiles = unmarshalFromKMZ.body().decl(arrayListClass.boxify().narrow(kmlClass.boxify()),
				"kmlfiles", JExpr._new(arrayListClass.boxify().narrow(kmlClass.boxify())));
		final JBlock while1 = unmarshalFromKMZ.body()._while(varEntries.invoke("hasMoreElements")).body();
		final JVar varEntry = while1.decl(zipEntryClass, "entry",
				JExpr.cast(zipEntryClass, varEntries.invoke("nextElement")));
		while1._if(varEntry.invoke("getName").invoke("contains").arg("__MACOSX")
				.cor(varEntry.invoke("getName").invoke("contains").arg(".DS_STORE")))._then()._continue();
		final JVar entryName = while1.decl(stringClass, "entryName",
				urlDecoderClass.boxify().staticInvoke("decode").arg(varEntry.invoke("getName")).arg("UTF-8"));
		...
		final JVar varUnmarshal = while1.decl(kmlClass, "unmarshal",
				kmlClass.boxify().staticInvoke("unmarshal").arg(varIn));
		...
		// JVar varOut = unmarshalFromKMZ.body().decl(zipEntryClass, "out",
		// JExpr._new(zipOutputStreamClass).arg(varName));

	}

	private void generateMarshalKmz(final ClassOutlineImpl cc) {
		// public boolean marshalKmz(@NotNull String name, Kml... additionalFiles)
		// throws IOException {
		...
		// return false;
		// }
		final JMethod generateMarshalKmz = cc.implClass.method(JMod.PUBLIC, cc.implClass.owner().BOOLEAN,
				"marshalAsKmz");
		...
		final JVar varOut = generateMarshalKmz.body().decl(zipOutputStreamClass, "out",
				JExpr._new(zipOutputStreamClass).arg(JExpr._new(fileOutputStreamClass).arg(varName)));
		generateMarshalKmz.body().add(varOut.invoke("setComment")
				.arg("KMZ-file created with Java API for KML. Visit us: http://code.google.com/p/javaapiforkml/"));
		generateMarshalKmz.body()
				.add(JExpr._this().invoke(createAddToKmzFile).arg(JExpr._this()).arg(varOut).arg(JExpr.TRUE));
	...
	private void generateUnmarshalMethodPlain(final ClassOutlineImpl cc, final JClass invokeMarshalWith) {
		...
		generateUnMarshallerFromString.javadoc().add("with the exception that it transforms a "
				+ invokeMarshalWith.name() + " into a graph of Java objects. \n");
		generateUnMarshallerFromString.javadoc().trimToSize();
		...
		final JVar localUnmarshaller = tryStringBlock.body().decl(jaxbUnmarshallerClass, "unmarshaller",
				jaxbContextClass.boxify().staticInvoke("newInstance").arg(JExpr.direct("Kml.class"))
						.invoke("createUnmarshaller"));

		final JVar inputsource = tryStringBlock.body().decl(inputsourceClass, "input",
				JExpr._new(inputsourceClass).arg(stringunmarshallVar));
		final JVar saxsource = tryStringBlock.body().decl(saxsourceClass, "saxSource", JExpr._new(saxsourceClass)
				.arg(JExpr._new(namespaceFilterXMLReaderclass).arg(JExpr.FALSE)).arg(inputsource));
		final JVar decl = tryStringBlock.body().decl(kmlClass, "jaxbRootElement",
				JExpr.cast(kmlClass, JExpr.invoke(localUnmarshaller, "unmarshal").arg(saxsource)));
		tryStringBlock.body()._return(decl);
		tryStringBlock._catch(saxExceptionClass.boxify()).body().directStatement("_x.printStackTrace();");
		tryStringBlock._catch(parserConfigurationExceptionClass.boxify()).body()
				.directStatement("_x.printStackTrace();");
	...
	private void generateUnmarshalMethodFromString(final ClassOutlineImpl cc, final JType invokeMarshalWith) {
		...
		generateUnMarshallerFromString.javadoc().add("Similar to the other unmarshal methods \n\n");
		generateUnMarshallerFromString.javadoc()
				.add("with the exception that it transforms a String into a graph of Java objects. \n");
		generateUnMarshallerFromString.javadoc().trimToSize();
		...
		final JVar inputsource = tryStringBlock.body().decl(inputsourceClass, "input",
				JExpr._new(inputsourceClass).arg(JExpr._new(stringReaderClass).arg(stringunmarshallVar)));
		final JVar saxsource = tryStringBlock.body().decl(saxsourceClass, "saxSource", JExpr._new(saxsourceClass)
				.arg(JExpr._new(namespaceFilterXMLReaderclass).arg(JExpr.FALSE)).arg(inputsource));
		final JVar decl = tryStringBlock.body().decl(kmlClass, "jaxbRootElement",
				JExpr.cast(kmlClass, JExpr.invoke(localUnmarshaller, "unmarshal").arg(saxsource)));
	...
	private void generateUnmarshalFileWithOptionalValidate(final ClassOutlineImpl cc, final JMethod generateValidate) {
		final JMethod generateUnMarshallerFileFile = cc.implClass.method(JMod.PUBLIC | JMod.STATIC, kmlClass,
				"unmarshal");
		generateUnMarshallerFileFile.javadoc().add("KML to Java\n");
		generateUnMarshallerFileFile.javadoc()
				.add("KML given as a file object is transformed into a graph of Java objects.\n");
		generateUnMarshallerFileFile.javadoc()
				.add("The boolean value indicates, whether the File object should be validated \n");
		generateUnMarshallerFileFile.javadoc()
				.add("automatically during unmarshalling and be checked if the object graph meets \n");
		...
		final JVar inputsource = tryBlock.body().decl(inputsourceClass, "input",
				JExpr._new(inputsourceClass).arg(JExpr._new(fileReaderClass).arg(fileunmarshallVar)));
		final JVar saxsource = tryBlock.body().decl(saxsourceClass, "saxSource", JExpr._new(saxsourceClass)
				.arg(JExpr._new(namespaceFilterXMLReaderclass).arg(validateVar)).arg(inputsource));
		final JVar decl = tryBlock.body().decl(kmlClass, "jaxbRootElement",
				JExpr.cast(kmlClass, JExpr.invoke(localUnmarshallerFile, "unmarshal").arg(saxsource)));
		tryBlock.body()._return(decl);
		tryBlock._catch(saxExceptionClass.boxify()).body().directStatement("_x.printStackTrace();");
		...
		generateUnMarshallerFile.javadoc()
				.add("KML given as a file object is transformed into a graph of Java objects.\n");
		...
		generateUnMarshallerFile.javadoc()
				.add("with the exception that the File object is not validated (boolean is false). ");
	...
	private JMethod generateValidateMethod(final ClassOutlineImpl cc) {
		final JMethod generateValidate = cc.implClass.method(JMod.PRIVATE | JMod.STATIC, cc.implClass.owner().BOOLEAN,
				"validate");
		...
		final JVar schemaFactoryVar = tryValidateBlock.body().decl(schemaFactoryClass, "sf", schemaFactoryClass.boxify()
				.staticInvoke("newInstance").arg(xmlConstantsClass.boxify().staticRef("W3C_XML_SCHEMA_NS_URI")));
		final JVar schemaFileVar = tryValidateBlock.body().decl(fileClass, "schemaFile",
				JExpr._new(fileClass).arg(schemaLocationVar));
		final JVar schemaVar = tryValidateBlock.body().decl(schemaClass, "schema",
				schemaFactoryVar.invoke("newSchema").arg(schemaFileVar));
