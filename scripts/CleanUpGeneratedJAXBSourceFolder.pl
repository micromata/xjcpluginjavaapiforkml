...
sub removeJAXBElementForOneJavaFile {
	...
	#   $currentJavaFile[$i] =~ s/(.*? this.getCoordinates\(\).add\()(coordinates)(\);)/$1new Coordinate($2)$3/g;	
		...
		# change SnippetDeprecated into a String
		#$currentJavaFile[$i] =~ s/(.*?)(final SnippetDeprecated aSnippetProperty,)(.*?)/$1final String aSnippetProperty,$3/g;
		#$currentJavaFile[$i] =~ s/(.*?)(protected SnippetDeprecated aSnippetProperty;)(.*?)/$1protected String aSnippetProperty;$3/g;
		#$currentJavaFile[$i] =~ s/(.*? public )(SnippetDeprecated)( [getASnippetProperty|createSnippetDeprecated].*?)/$1String$3/g;
		#$currentJavaFile[$i] =~ s/(.*? public )(SnippetDeprecated)( getASnippetProperty.*?)/$1String$3/g;
		#$currentJavaFile[$i] =~ s/(.*? public void setASnippetProperty\()(SnippetDeprecated)( value.*?)/$1String$3/g;
		#$currentJavaFile[$i] =~ s/(.*?)(\@XmlElementRef.*? SnippetDeprecated.class\))(.*?$)//g;
		#$currentJavaFile[$i] =~ s/(.*?)(return new )(SnippetDeprecated\()(value)(\))(.*?\n)/$1$2 String\($4\);\n/g;

		#$currentJavaFile[$i] =~ s/(.*? public )(SnippetDeprecated)( createSnippetDeprecated.*?)/$1JAXBElement<String>$3/g;
		#$currentJavaFile[$i] =~ s/(.*?)(return new )(SnippetDeprecated\()(value)(\))(.*?\n)/$1$2 JAXBElement<String>(_Snippet_QNAME, String.class, null, value);\n/g;
			
		# change ScaleDeprecated into a Double
		#$currentJavaFile[$i] =~ s/(.*?)(final ScaleDeprecated scale,)(.*?)/$1final Double scale,$3/g;
		#$currentJavaFile[$i] =~ s/(.*?)(protected ScaleDeprecated scale;)(.*?)/$1protected Double scale;$3/g;
		#$currentJavaFile[$i] =~ s/(.*? public )(ScaleDeprecated)( [getScale|createScaleDeprecated].*?)/$1Double$3/g;
		#$currentJavaFile[$i] =~ s/(.*? public )(ScaleDeprecated)( getScale.*?)/$1Double$3/g;
		#$currentJavaFile[$i] =~ s/(.*? public void setScale\()(ScaleDeprecated)( value.*?)/$1Double$3/g;
		#$currentJavaFile[$i] =~ s/(.*?)(\@XmlElementRef.*? ScaleDeprecated.class\))(.*?$)//g;
		#$currentJavaFile[$i] =~ s/(.*?)(return new )(ScaleDeprecated\()(value)(\))(.*?\n)/$1$2 Double\($4\);\n/g;

		#$currentJavaFile[$i] =~ s/(.*? public )(ScaleDeprecated)( createScale.*?)/$1JAXBElement<Double>$3/g;
		#$currentJavaFile[$i] =~ s/(.*?)(return new )(ScaleDeprecated\()(value)(\))(.*?\n)/$1$2 JAXBElement<Double>(_Scale_QNAME, Double.class, null, value);\n/g;
	
		#if ($currentJavaFile[$i] =~ /(.*? public AltitudeModeEnumTypeDeprecated createAltitudeModeEnumTypeDeprecated.*?$)/) {
		#	$currentJavaFile[$i-5] =  "";
		#	$currentJavaFile[$i-4] =  "";
		#	$currentJavaFile[$i-3] =  "";
		#	$currentJavaFile[$i-2] =  "";
		...
		#	$currentJavaFile[$i+2] = "";
		#}
		#clean up the ObjectFactory related stuff
		#$currentJavaFile[$i] =~ s/(.*?)(return new String)(.*?)String.class(.*?\n)/$1$2(value);\n/g;
